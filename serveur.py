# serveur.py (Corrigé pour le mode --onedir)
...
try:
    ...
    if getattr(sys, 'frozen', False):
        # En mode --onedir, le chemin de l'application et des données est le dossier de l'exécutable.
    ...
    # On spécifie les chemins pour la robustesse.
    app = Flask(__name__,
                template_folder=os.path.join(application_path, 'templates'),
                static_folder=os.path.join(application_path, 'static'))

    # --- CONSTANTES ---
    APP_VERSION = "1.1.2.1" # Version de base de votre serveur
    ...
    # --- (Le reste de vos fonctions utilitaires reste ici, inchangé) ---
    ...
    # --- (Toutes vos routes existantes de /add_camera_form à /api/status restent ici) ---
        ...
        try:
            ...
            if latest_v > current_v:
                ...
                update_info.update({
                    ...
                    "notes": "Votre application est à jour."
                })
        except Exception as e:
            print(f"[UPDATE PAGE] Erreur: {e}")
